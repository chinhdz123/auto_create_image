...
from image_aere_nhung import image_x, image_y
import os
...
thresh_change = 20
save_path = r"C:\Users\Nhung\Desktop\data augmentation"
saved_images = [name for name in os.listdir(save_path)]

path_folder_background = r"C:\Users\Nhung\Downloads\Data.segmentation.Version 3.yolo\val\images"
images_names = [name for name in os.listdir(path_folder_background)] #lấy tên tất cả ảnh trong folder

images_names_remain = list(set(images_names) - set(saved_images))

for i in range(len(images_names_remain)):
    #nếu i % thresh_change == 0 và i != 0 thì đổi conversation
    if i % thresh_change == 0 and i != 0:
        change_conv()
    name = images_names_remain[i]
    prompt = random.choice(prompts)
    success = auto_gen_image(prompt, name)
    if not success:
        print("Image generation failed.")
        break
    print("Generated image for background:", images_names_remain[i])
